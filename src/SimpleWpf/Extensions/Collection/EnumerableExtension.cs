...
namespace SimpleWpf.Extensions.Collection
{
    ...
    public static class EnumerableExtension
    {
        ...
        {
            var transformedList = collection.Select(x => transform(x)).ToList();

            var result = "";

            for (int index = 0; index < transformedList.Count; index++)
            {
                if (index == transformedList.Count - 1)
                    result += transformedList[index];

                else
                    result += transformedList[index] + separator;
            }
